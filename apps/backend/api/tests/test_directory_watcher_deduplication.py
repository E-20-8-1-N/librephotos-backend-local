...
class PhotoDeduplicationTests(PhotoDeduplicationMixin, TestCase):
    ...
    def test_copied_file_replaces_missing_canonical_path(self):
        original_path = self.create_image("missing-original.png")
        copied_path = str(Path(self.temporary_directory.name, "surviving-copy.png"))
        shutil.copyfile(original_path, copied_path)
        ...
        self.assertEqual(original_photo.pk, copied_photo.pk)
        self.assertEqual(File.objects.get().path, copied_path)
        self.assertFalse(copied_photo.missing_on_disk())
    ...
    def test_main_file_prevents_false_missing_result_without_m2m_relation(self):
        path = self.create_image("main-file-only.png")
        file = File.create(path, self.user)
        photo = self.create_photo("legacy-image-hash", main_file=file)

        self.assertFalse(photo.missing_on_disk())
        self.assertEqual(photo.main_file_id, file.pk)
