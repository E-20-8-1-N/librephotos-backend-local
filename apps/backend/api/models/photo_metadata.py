...
class PhotoMetadata(models.Model):
    ...
    def extract_exif_data(cls, photo, commit=True, overwrite=True, try_sidecar=True):
        ...
        caption_tag = Tags.KEYS_DESCRIPTION if photo.video else Tags.IMAGE_DESCRIPTION
        caption_value = tag_values.get(caption_tag)
        ...
        keywords = []
        for value in (xmp_subject, iptc_keywords):
            for keyword in cls._normalize_keywords(value) or []:
